...
export const onRequestGet = async ({ request, env }) => {
    ...
    // Comparamos expiración en SEGUNDOS para evitar problemas de formato/huso
  ...
  `).bind(sid).all();
    ...
    return json({
        ...u,
        perms: (u.role === 'admin'
            ? { all: true, inventory: true, quotes: true, settings: true, reports: true, pos: true }
            : { pos: true, quotes: true, inventory: true })
    });
};
