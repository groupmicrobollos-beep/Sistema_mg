  ...
  mount(root){
    ...
    async function saveUser(mode="close"){
      ...
      if (existing){
        ...
        if (pass) existing.passHash = await hashPassword(pass);
        logAudit("user.update", { id:data.id, username:data.username });
      } else {
        const nu = { ...data, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), passHash: "" };
        if (pass) nu.passHash = await hashPassword(pass);
        users.push(nu);
        logAudit("user.create", { id:nu.id, username:nu.username });
      }
      save(CFG_USERS_KEY, users);
      // 🔔 avisar al sistema
      window.CFG?.setUsers(users);
