...
function findSupplier(id) { return load(INV_SUPPLIERS_KEY, []).find(s => s.id === id) || null; }
...
<section data-page="inventory" class="space-y-6 text-[13px]">
  ...
  mount(root) {
    ...
    // Estado
    let items = load(INV_ITEMS_KEY, []);
    let suppliers = load(INV_SUPPLIERS_KEY, []);
    let buyList = load(INV_LIST_KEY, []);

    // 🧯 FIX TDZ: declarar viewItems ANTES de refreshItems()
    let viewItems = [];
    ...
    listWrap.addEventListener("input", (e) => {
      const row = e.target.closest('[data-blid]'); if (!row) return;
      const id = row.getAttribute("data-blid");
      const line = buyList.find(b => b.id === id); if (!line) return;
    ...
    function saveItem(mode="close"){
      ...
      existing ? Object.assign(existing, data) : items.push(data);
      ...
      if (mode==="new"){ openItem(null); } else { hide(itemModal); }
    }
    function delItem(id){
      if (!confirm("¿Eliminar insumo?")) return;
      items = items.filter(i=>i.id!==id);
      buyList = buyList.filter(b=>b.itemId!==id);
      save(INV_ITEMS_KEY, items); save(INV_LIST_KEY, buyList);
      refreshItems();
    ...
    function toggleNeed(id){
      ...
      if (exists) buyList = buyList.filter(b=>b.itemId!==id);
      else buyList.push({ id: rid("bl"), itemId: id, qty: Math.max(1,it.min||1), note:"", supplierId: it.supplierId||"" });
      save(INV_LIST_KEY, buyList);
      paintBuyList(); refreshKPIs();
    }
    ...
    function paintBuyList(){
      if (!buyList.length){
        listWrap.innerHTML=""; listEmpty.classList.remove("hidden"); return;
      }
            ...
            <div class="mt-2 space-y-1">
              ${arr.map(b=>`
                <div class="flex items-center gap-2" data-blid="${b.id}">
                  ...
                  <input type="number" min="1" class="bl-qty w-16 h-7 px-2 rounded bg-white/10 border border-white/10 text-right" value="${b.qty}"/>
    ...
    function paintSuppliers(){
      if (!suppliers.length){
        rowsSuppliers.innerHTML=""; emptySuppliers.classList.remove("hidden");
      } else {
        ...
        rowsSuppliers.innerHTML = suppliers.map(s=>`
          <tr class="hover:bg-white/5">
            ...
            <td>${s.phone || "-"}</td>
            <td>${s.email || "-"}</td>
            <td class="max-w-[220px] truncate">${(s.tags||[]).join(", ") || "-"}</td>
    ...
    function refreshSuppliers(){ paintSuppliers(); repaintSupplierFilter(); }
    ...
    function saveSupplier(mode="close"){
      const data = readSupplierForm(suppForm);
      if (!data.name) return toast("Nombre del proveedor obligatorio","error");
      const ex = suppliers.find(s=>s.id===data.id);
      ex ? Object.assign(ex,data) : suppliers.push(data);
      save(INV_SUPPLIERS_KEY, suppliers);
      refreshSuppliers();
      toast("Proveedor guardado ✅","success");
      if (mode==="new"){ openSupplier(null); } else { hide(suppModal); }
    }
    function delSupplier(id){
      if (!confirm("¿Eliminar proveedor? (los insumos quedarán sin proveedor)")) return;
      suppliers = suppliers.filter(s=>s.id!==id);
      items = items.map(it => it.supplierId === id ? { ...it, supplierId: "" } : it);
      save(INV_SUPPLIERS_KEY, suppliers); save(INV_ITEMS_KEY, items);
      refreshSuppliers(); refreshItems();
