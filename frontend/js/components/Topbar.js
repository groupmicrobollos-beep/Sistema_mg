...
export function Topbar() {
  ...
  const name = u?.name || u?.username || "Invitado";

  return /*html*/`
    <header class="sticky top-0 z-10 glass px-3 py-2 flex items-center gap-2">
      <button id="btnMenu" class="md:hidden inline-flex items-center px-3 py-2 rounded hover:bg-white/10">☰</button>
...
export function mountTopbar(root) {
  ...
  if (!el) return;
  subscribe("auth", (auth) => {
    el.textContent = auth?.user?.name || auth?.user?.username || "Invitado";
  });
}
